...
const Profile: React.FC = () => {
  ...
  const removeOffering = (t: string) =>
    setOfferings((prev) => prev.filter((x) => x !== t));
  ...
  const removeSeeking = (t: string) =>
    setSeeking((prev) => prev.filter((x) => x !== t));
      ...
      <Container maxWidth="sm">
        <Paper sx={{ p: 3 }}>
          ...
          {step === 1 && (
            <Box>
              ...
              <Box sx={{ display: '柔性', gap: 1, flexWrap: 'wrap', mt: 1 }}>
                {presetTopics.map((t) => (
                  <Chip
                    ...
                    variant={interests.includes(t) ? 'filled' : 'outlined'}
              ...
              <Box sx={{ display: 'flex', gap: 1, mt: 2 }}>
                ...
                />
                <Button variant="outlined" onClick={addCustomInterest}>
                  添加到感兴趣
                </Button>
              </Box>
            </Box>
            ...
            <Box>
              ...
              <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mt: 1 }}>
                {presetTopics.map((t) => (
                  <Chip
                    ...
                    label={t}
                    clickable
                    color={expertise.includes(t) ? 'primary' : 'default'}
                    variant={expertise.includes(t) ? 'filled' : 'outlined'}
              ...
              <Box sx={{ display: 'flex', gap: 1, mt: 2 }}>
                <TextField
                  ...
                  onChange={(e) => setCustomInput(e.target.value)}
                />
                <Button variant="outlined" onClick={addCustomExpertise}>
                  添加到擅长
                </Button>
              </Box>
            </Box>
            ...
            <Box>
              ...
              <Box sx={{ display: 'flex', gap: 1 }}>
                <TextField
                  fullWidth
                  label="添加条目"
                  value={offeringsInput}
                  onChange={(e) => setOfferingsInput(e.target.value)}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      e.preventDefault();
                      addOffering();
                ...
                />
                <Button variant="outlined" onClick={addOffering}>
                  添加
                </Button>
              </Box>
              ...
              <Box sx={{ display: 'flex', gap: 1 }}>
                ...
                />
                <Button variant="outlined" onClick={addSeeking}>
                  添加
                </Button>
              </Box>
              <Box sx={{ mt: 1, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
          ...
          {step === 5 && (
            <Box>
              ...
              <Box sx={{ mt: 2 }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  {offerings.length ? offerings.join('、') : '未填写'}
                </Typography>
                <Typography variant="body2" sx={{ mt: 1 }}>
                  寻找帮助
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  {seeking.length ? seeking.join('、') : '未填写'}
                </Typography>
              </Box>
            </Box>
          )}
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 3 }}>
            ...
            {step < steps.length - 1 ? (
              <Button variant="contained" onClick={proceedNext}>
                下一步
              </Button>
            ) : (
