...
const BulkWeeklyUpload: React.FC = () => {
  ...
  const [results, setResults] = useState<
    Array<{ index: number; success: boolean; message: string; id?: any }>
  >([]);
      ...
      <Container maxWidth="md">
        <Paper sx={{ p: 3 }}>
          ...
          <Typography variant="body2" color="text.secondary">
            粘贴 JSON 数组，包含 name、title、quote、detail、episode 字段
          </Typography>
          <TextField
            fullWidth
            multiline
          ...
          <Box sx={{ display: 'flex', gap: 1, mt: 2 }}>
            ...
            {!!validCount && (
              <Chip size="small" label={`待上传：${validCount}`} />
            )}
          </Box>
          <List sx={{ mt: 2 }}>
            {items.map((it, idx) => {
                ...
                <ListItem key={idx} sx={{ borderBottom: '1px solid #eee' }}>
                  ...
                  {r ? (
                    r.success ? (
                      ...
                      <Chip size="small" color="error" label={r.message} />
                    )
                  ) : (
