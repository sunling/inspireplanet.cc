...
const MyMeetups: React.FC = () => {
  ...
  const deleteMeetup = async (meetupId: string) => {
    ...
    try {
      // 模拟API调用
      // const response = await fetch(`/.netlify/functions/meetupHandler?id=${meetupId}&createdBy=${currentUser?.username}`, {
      //   method: 'DELETE'
      // });
      // const data = await response.json();
  ...
  const getFilteredMeetups = () => {
    ...
    return allMeetups.filter((meetup) => {
      ...
      if (currentStatus === 'active') {
        return meetup.status === 'active' && meetupDate > new Date();
      }
          ...
          <>
            {getFilteredMeetups().length === 0 ? (
              <Paper elevation={3} sx={{ p: 6, textAlign: 'center' }}>
                <Typography variant="h5" sx={{ mb: 2 }}>
                  暂无{getStatusText(currentStatus)}活动
                </Typography>
                <Typography variant="body1">
                  您还没有{getStatusText(currentStatus)}的活动，快去发起一个吧！
                </Typography>
              </Paper>
            ) : (
              <Grid
