...
// 创建RSVP
async function createRSVP(event, headers) {
  try {
...
// 获取RSVP列表
async function getRSVPs(event, headers) {
  try {
...
// 更新RSVP状态
async function updateRSVP(event, headers) {
  try {
    const params = event.queryStringParameters || {}
    const { id } = params
...
// 取消RSVP
async function deleteRSVP(event, headers) {
  try {
    const params = event.queryStringParameters || {}
    const { id, meetup_id, wechat_id } = params

    let query = supabase.from('meetup_rsvps')
